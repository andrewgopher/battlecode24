...
public strictfp class Communicator {
    ...
    public static void processSharedArray(RobotController rc, MapLocation[] sharedAllyFlagInfo, MapLocation[] sharedEnemyFlagInfo) throws GameActionException {
        for (int i = 0; i < 3; i ++) {
    ...
    public static void updateEnemyFlagLocations(RobotController rc, MapLocation[] sharedEnemyFlagInfo) throws GameActionException {
        FlagInfo[] nearbyFlags = rc.senseNearbyFlags(-1);
        for (int i = 0; i < 3; i ++) {
            if (sharedEnemyFlagInfo[i] != null && rc.canSenseLocation(sharedEnemyFlagInfo[i])) {
                boolean isThere= false;
                for (FlagInfo flag : nearbyFlags) {
                    if (flag.getLocation().equals(sharedEnemyFlagInfo[i])) {
                        isThere = true;
                        break;
                    }
                }
                if (!isThere) {
                    eraseFirstLocation(rc, enemyFlagLocationsStart, enemyFlagLocationsStart+3*12, sharedEnemyFlagInfo[i]);
                    sharedEnemyFlagInfo[i] = null;
                }
            }
        }
        for (FlagInfo flag : nearbyFlags) { //TOOD: only add untouched
            if (flag.getTeam() != rc.getTeam() && rc.getRoundNum() >= GameConstants.SETUP_ROUNDS && !flag.isPickedUp()) {
                writeEnemyFlagLocation(rc, flag.getLocation());
            }
        }
    }
    ...
    public static void writeEnemyFlagLocation(RobotController rc, MapLocation loc) throws GameActionException {
        writeLocationAtFirstEmptyIfUnique(rc, enemyFlagLocationsStart, enemyFlagLocationsStart+3*12, loc);
    }

    public static int getNumDefenders(RobotController rc) throws GameActionException {
        return interpretNumber(rc, numDefendersStart, 6);
    }
